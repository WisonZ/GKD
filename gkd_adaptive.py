...
def gkd_loss(logits_student, logits_teacher, target, alpha, beta, lamda, temperature, rate,frequency=100):
    ...
    logit_length = sorted_prob_teacher.shape[0]
    for idx in range(1, logit_length, frequency):
        if torch.sum(sorted_prob_teacher[:idx], dim=0) >= rate * batch_size:
            hcm_n = idx
            break
    ##
...
def gkd_loss_student_base(logits_student, logits_teacher, target, alpha, beta, lamda, temperature, rate,frequency=100):
